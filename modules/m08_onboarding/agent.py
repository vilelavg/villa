...
ONBOARDING_STEPS = [
    ...
    {"step": 7, "name": "InLead configurado com mapeamento de campos", "owner": "operacional", "sla_days": 2},
...
class M08Onboarding(BaseModule):
    ...
    async def execute(self, message: str, db: AsyncSession, user: User | None = None, client_slug: str | None = None, context: dict | None = None) -> dict:
        ...
        for step in ONBOARDING_STEPS:
            status = "✅" if step["step"] in completed_steps else "⏳" if step["step"] == current_step else "⬜"
        ...
        if start_date:
            ...
            for step in ONBOARDING_STEPS:
                if step["step"] not in completed_steps and step["step"] <= current_step:
                    expected_date = start + timedelta(days=sum(s["sla_days"] for s in ONBOARDING_STEPS[:step["step"]]))
                    if datetime.utcnow() > expected_date:
                        days_late = (datetime.utcnow() - expected_date).days
                        sla_alerts.append(f"Etapa {step['step']} ({step['name']}) atrasada {days_late} dia(s)")
        ...
        await feedback_loop.record_decision(
            module=self.code, action="check_onboarding",
        ...
        return {
            "success": True,
            "message": msg,
            "data": {"client": client.slug, "current_step": current_step, "analysis": analysis, "sla_alerts": sla_alerts},
