...
from core.models import Client, Lead, LeadStatus, Campaign, ModuleCode, User
...
class M07Retroalimentacao(BaseModule):
    ...
    KEYWORDS = ["retroalimentação", "retroalimentacao", "feedback", "funil", "conversão", "conversao", "comercial e marketing", "por que não fechou", "motivo de perda"]

    async def can_handle(self, message: str, context: Optional[dict] = None) -> float:
        if context and "retroalimentacao" in context.get("event_type", ""): return 0.9
    ...
    async def execute(self, message: str, db: AsyncSession, user: Optional[User] = None, client_slug: Optional[str] = None, context: Optional[dict] = None) -> dict:
        ...
        # Dados do funil
        leads_q = await db.execute(select(Lead).where(Lead.client_id == client.id).where(Lead.created_at >= week_ago))
        leads = leads_q.scalars().all()

        total = len(leads)
        qualified = sum(1 for l in leads if l.status in (LeadStatus.QUALIFIED, LeadStatus.SCHEDULED, LeadStatus.PROPOSAL, LeadStatus.WON))
        ...
        loss_reasons = [l.disqualification_reason or "não informado" for l in leads if l.status == LeadStatus.LOST]

        # Cruzar com campanhas
        winning_utms = [l.utm_campaign for l in leads if l.status == LeadStatus.WON and l.utm_campaign]
        losing_utms = [l.utm_campaign for l in leads if l.status == LeadStatus.LOST and l.utm_campaign]

        memory = await feedback_loop.build_context(module=self.code, action="retroalimentacao", client_slug=client.slug)

        response = await self.ask_claude(
            message=ANALYSIS_PROMPT.format(
                client_name=client.name, specialty=client.specialty or "odontologia",
                total_leads=total, qualified=qualified, qual_rate=round(qualified/total*100,1) if total else 0,
                scheduled=scheduled, won=won, avg_ticket=f"{avg_ticket:,.2f}", lost=lost,
        ...
        await feedback_loop.record_decision(
            module=self.code, action="retroalimentacao",
            input_data={"client": client.slug, "total_leads": total, "won": won, "lost": lost},
            output_data=analysis, client_slug=client.slug,
        )
